...
function check_dependencies_and_install_secor {
  if [ -z "${DEP}" ]; then
    echo -e "${color_yellow}Installing Secor on Node - ${MASTER_PUBLIC_DNS} - in ${CLUSTER_NAME}${color_norm}"
    script=${PEG_ROOT}/install/secor/install_secor.sh
