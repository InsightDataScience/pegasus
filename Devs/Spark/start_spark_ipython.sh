...
ssh -o "StrictHostKeyChecking no" -i $PEMLOC ubuntu@$(head -n 1 tmp/$INSTANCE_NAME/public_dns) 'bash -s' < Spark/setup_ipython.sh $AWS_ACCESS_KEY_ID $AWS_SECRET_ACCESS_KEY
